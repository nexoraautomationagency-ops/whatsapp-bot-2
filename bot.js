...
const I18N = {
    [LANG.EN]: {
        ...
        'start.cancelled': 'Session cancelled!\n\n_Whenever you are ready, just type menu._',
        ...
        'month.ask': 'Which *month* are you paying for (e.g., April)?\n\n_🔙 (Type back to edit)_',
        ...
        'back.new.phone': 'What is the *WhatsApp number* to add to the group?',
        'back.new.grade': 'Which *Grade* are you in (6-11)?',
        'back.new.month': 'Which *month* (e.g. April)?',
        ...
        'back.old.month': 'Which *month* (e.g. April)?',
    ...
    [LANG.SI]: {
        ...
        'start.cancelled': 'Session එක cancel කළා. _ආයෙත් පටන් ගන්න ඕන වුණාම menu කියලා එවන්න._',
        ...
        'email.ask': 'ඔයාගේ *email address* එක එවන්න:\n\n_🔙 (වෙනස් කරන්න back එවන්න)_',
        ...
        'month.ask': 'ඔයා payment එක කරන්නේ මොන *month* එකටද (Eg: April)?\n\n_🔙 (වෙනස් කරන්න back එවන්න)_',
        ...
        'confirm.preview': '📋 *ඔයාගේ details සියල්ල මෙන්න:*\n\n**Name:** {{name}}\n**School:** {{school}}\n**Student ID:** {{idNumber}}\n**Month:** {{month}}\n**Grade:** {{grade}}\n**Tutes:** {{tutes}}{{addressLine}}\n\n_සියල්ල නිවැරදි නම් "yes" කියලා reply කරන්න. ඔක්කොම cancel කරන්න "menu" එවන්න._',
        ...
        'back.new.month': 'අදාළ *month* එක (Eg: April) එවන්න:',
        ...
        'back.old.month': 'අදාළ *month* එක (Eg: April) එවන්න:',
...
function getUserLang(from) {
    ...
    const saved = userLangPref.get(from);
    return saved === LANG.SI ? LANG.SI : LANG.EN;
}
...
• \`getgroups\` - List all your WhatsApp groups.`;
        ...
        if (lowerBody === 'cancel') {
            ...
            if (savedLang) {
                ...
                resetUser(from);
                userData.set(from, { lang, lastSeen: Date.now() });
                userStates.set(from, STATES.START);
                await sendWA(from, t(lang, 'start.cancelled'));
                return await sendMainMenu(from);
            }
            const previous = userData.get(from) || {};
            const lang = getUserLang(from);
